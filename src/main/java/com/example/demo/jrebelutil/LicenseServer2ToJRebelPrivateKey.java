package com.example.demo.jrebelutil;
...
public class LicenseServer2ToJRebelPrivateKey {
    ...
    private static PrivateKey a() {
        final PKCS8EncodedKeySpec pkcs8EncodedKeySpec =
                new PKCS8EncodedKeySpec(LicenseServer2ToJRebelPrivateKey.c);
        try {
    ...
    public static byte[] a(final byte[] array) {
        try {
            final Signature instance =
                    Signature.getInstance("SHA1withRSA", LicenseServer2ToJRebelPrivateKey.d);
            instance.initSign(a());
